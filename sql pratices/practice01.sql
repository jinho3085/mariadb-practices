...
-- 문제6.
-- 현재 부서 매니저는 몇명이나 있나요?(역임 매너저는 제외)
select count(distinct emp_no) as '현재 매니저 수'
from dept_manager
where to_date = '9999-01-01';
...
select count(*) as '120000 이상 급여 직원 수'
	from salaries
    where to_date = '9999-01-01'
    and salary >= 120000;
...
select count(distinct emp_no) as 'Engineer 수'
	from titles
...
SELECT title AS '직책', from_date AS '시작일', to_date AS '종료일'
FROM titles
WHERE emp_no = 13250
